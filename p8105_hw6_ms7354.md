...
`The plot above shows that the weather bootstrap samples for R-squared which are mostly clustered between 0.935 and 0.945. The shape is sysmetric and smooth. The model consistently explains about 93.5% - 94.5% of variation on tmax, with little variability across bootstrap samples.`
...
`The plot above is centered around -0.006 and stays consistently negative which means that the products of the coefficients (Beta1 and Beta2) are stable accorss samples and suggests and inverse combined effect the two predictors.`
